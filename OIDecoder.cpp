...
EncodingType dbt::OIDecoder::getEncodingType(OIInstType InstType) {
  switch(InstType) {
...
extern void dbt::OIDecoder::fillFields(OIInst& I, EncodingType E, Word W) {
  switch (E) {
...
bool CallZero;
OIInst dbt::OIDecoder::decode(uint32_t CodedInst) {
  Word W;
  W.asI_ = CodedInst;
  ...
  switch(Op) {
    ...
    case 0b100010:
      ...
      if (Ext == 0b1101  ) I.Type = OIInstType::Cueqd;
      ...
      if (Ext == 0b100100) I.Type = OIInstType::Movts;
  ...
  dbt::OIDecoder::fillFields(I, dbt::OIDecoder::getEncodingType(I.Type), W);
...
std::array<uint32_t, 2> dbt::OIDecoder::getPossibleTargets(uint32_t PC, OIInst Inst) {
  switch (Inst.Type) {
...
bool dbt::OIDecoder::isControlFlowInst(OIInst Inst) {
  auto T = dbt::OIDecoder::getPossibleTargets(1, Inst);
  if (T[0] == 5 && T[1] == 5) 
    return false;
  return true;
}

bool dbt::OIDecoder::isIndirectBranch(OIInst Inst) {
  switch (Inst.Type) {
